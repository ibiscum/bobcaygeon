...
func TestDAAPParse(t *testing.T) {
	...
	if len(parsed) != 4 {
		t.Errorf(fmt.Sprintf("Expected: 4 entries\r\n Got: %d", len(parsed)))
	}
	val, ok := parsed["dmap.itemkind"]
	if !ok {
		t.Error("Expected to have dmap.itemkind key")
	}
	if val.(uint8) != 2 {
	...
	if !ok {
		t.Error("Expected to have daap.songalbum key")
	}
	...
	if !ok {
		t.Error("Expected to have dmap.itemname key")
	}
	...
	if !ok {
		t.Error("Expected to have daap.songartist key")
	}
...
func TestDAAPEncode(t *testing.T) {
	...
	if val.(string) != "Phantom Power" {
		t.Errorf(fmt.Sprintf("Expected: %v, Received: %s", "Phantom Power", val))
	}
	val, ok = parsed["dmap.itemname"]
	if !ok {
		t.Error("Expected to have dmap.itemname key")
